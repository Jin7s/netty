...
public class DefaultDnsRecordEncoder implements DnsRecordEncoder {
    ...
    static int calculateEcsAddressLength(int sourcePrefixLength) {
        // Guard against value <= Byte.SIZE
        return sourcePrefixLength == 0
                ? 0 : Math.max(1, (int) Math.rint((double) sourcePrefixLength / Byte.SIZE));
    }
    ...
    static byte padWithZeros(byte b, int leftOverBits) {
        int lastBit = Byte.SIZE - leftOverBits;
        for (int i = Byte.SIZE - 1; i >= lastBit; --i) {
            b &= ~(1 << i);
        }
        return b;
    }
